...
public class UserDAO {
     ...
     *
     * @param username Atributo que guarda el nombre de usuario del usuario a
    ...
    public void userRegister(String username, String email, String password, String role) {
        ...
        String insertar = "insert into usuario values('" + username + "','" + email + "',dbo.ENCRIPTA_PASS('" + password + "'),'" + role + "')";
     ...
     * @param password Atributo que la contraseña del usuario en la base de
     * datos
     * @param type Atributo que compara el tipo en la base de datos
     * @return
     */
    public String logIn(login user) {
        try {
            
            String login = "select nombreUsuario,contraseña,tipo from usuario where nombreUsuario='" + user.getEmail() + "' AND contraseña='" + user.getPassword() +"'";
     ...
     * Método para eliminar los usuarios
     *
     * @param table Atributo que hace referencia a la tabla donde se mostrará
